...
from flame_memory.ports import MetricsSink, UsageRecorder
...
class FlameManager:
    ...
    def trigger(self, agent_id: str | None = None, force: bool = False) -> list[ReflectionBatchRun]:
        if agent_id:
            ...
            agent_ids = [agent.id for agent in self.store.list_self_learning_agents()]
        runs: list[ReflectionBatchRun] = []
    ...
    def _build_session_record(self, agent_id: str, session_id: str) -> SessionRecord:
        ...
        for event in events:
            ...
            if event_type == "input" and not initial_input:
